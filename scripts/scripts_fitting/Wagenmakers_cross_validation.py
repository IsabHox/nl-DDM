...
def crossvalidation(subject):
    ...
    y=subdat['stratification']
    
    for i, (train_index, test_index) in enumerate(skf.split(subdat, y)):
        Xtrain, Xtest=subdat.iloc[train_index],subdat.iloc[test_index]
    ...
    # Instanciate models for fitting the accuracy condition
        ...
        results_df.to_csv(f'../../results/5crossval_Wagenmakers_noFatigue_{subject}_{i}.csv')
    return subject
